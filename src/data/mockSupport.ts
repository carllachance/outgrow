...
const LOW_RISK_RESPONSES = [
  'Let’s make today smaller: pick one regular meal and do a quick check-in afterward.',
...
const APPROVED_REDIRECT_TEMPLATES: Record<RiskCategory, string> = {
  medical:
    'I can’t provide diagnosis or treatment advice. We can focus on a simple routine like regular meals, hydration, and a short reflection.',
  compensatory:
  ...
  dependency_or_deterministic:
    'I can’t treat wearables as absolute rules or encourage dependence. We can use your data as a clue and choose one flexible habit for today.',
  none:
    'Let’s keep this simple: pick one gentle step you can repeat today.'
};
...
export const getHandResponse = (request: string) => {
  ...
  if (tier >= 3) {
    return {
      response:
        'I can’t help with tracking or optimization right now. Let’s focus on immediate safety: pause, breathe, and reach out to someone you trust or local crisis support if you might be in danger.',
  ...
  if (!postCheck.isSafe) {
    return {
      response:
        'Let’s keep this safe and kind. Pause and choose one supportive next step for the next hour.',
      mode: 'supportive_redirective' as const,
      classification,
      checklist: OUTGROW_AGENT_RESPONSE_CHECKLIST
    };
